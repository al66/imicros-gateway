...
module.exports = {
	...
	settings: {
		/** Secret for JWT */
        //##TODO: not save - settings are visible in ~node/services info
		JWT_SECRET: process.env.JWT_SECRET || "jwt-imicros-users-secret",
	...
	methods: {
         ...
         *
         * return value must include attribute access_token as expected by vue-authenticate
		 */
		transformEntity(user, withToken, token) {
			...
			if (user) {
				...
				//if (withToken)
				//	user.access_token = token || this.generateJWT(user);
			}
            if (withToken)
					access_token = token || this.generateJWT(user);
